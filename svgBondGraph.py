...
BlackandPrimary = ["#000000","#ff0000","#00ff00","#0000ff"]
def getBonds(svg,allowedColour=BlackandPrimary):
...
def renamePar(str,parRename={}):
    """ Rename parameters 
    """

    q = "'" # Quote mark
    oldName = parRename.keys()
    newstr = str
    for name in oldName:
        print(name)
        Name = q+name+q
        newName = q+parRename[name]+q
        print('Replacing',Name,'with', newName)
        newstr = newstr.replace(Name,newName)
    return newstr
        
def model(svg,convertR=False,convertCe=False,quiet=False,rename=True,parRename={}):
    ...
    ## Create the components - return as string
    strComp = makeComponents(sorted(compList),compPort,convertR=convertR,convertCe=convertCe,rename=rename)

    ## Modify if required
    print(parRename)
    if len(parRename)>0:
        strComp = renamePar(strComp,parRename=parRename)
